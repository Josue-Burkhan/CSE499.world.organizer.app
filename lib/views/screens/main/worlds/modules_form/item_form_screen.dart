...
class _ItemFormScreenState extends ConsumerState<ItemFormScreen> {
  ...
  // Links (Raw)
  // Links (Raw)
  List<ModuleLink> _rawCreatedBy = [];
  List<ModuleLink> _rawUsedBy = [];
  List<ModuleLink> _rawCurrentOwnerCharacter = [];
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          AutocompleteChips(
            label: 'Created By',
            initialValues: _rawCreatedBy.map((e) => e.name).toList(),
            onChanged: (values) => _updateLinksList(values, _rawCreatedBy, (l) => _rawCreatedBy = l),
            searchFunction: (q) => _search(q, 'character'),
          ),
          const SizedBox(height: 16),
          AutocompleteChips(
            label: 'Used By',
            initialValues: _rawUsedBy.map((e) => e.name).toList(),
            onChanged: (values) => _updateLinksList(values, _rawUsedBy, (l) => _rawUsedBy = l),
            searchFunction: (q) => _search(q, 'character'),
          ),
          const SizedBox(height: 16),
          AutocompleteChips(
            label: 'Current Owner',
            initialValues: _rawCurrentOwnerCharacter.map((e) => e.name).toList(),
            onChanged: (values) => _updateLinksList(values, _rawCurrentOwnerCharacter, (l) => _rawCurrentOwnerCharacter = l),
            searchFunction: (q) => _search(q, 'character'),
          ),
          const SizedBox(height: 16),
          AutocompleteChips(
          ...
          AutocompleteChips(
            ...
            onChanged: (values) => _updateLinksList(values, _rawPowerSystems, (l) => _rawPowerSystems = l),
          ...
          AutocompleteChips(
            ...
            onChanged: (values) => _updateLinksList(values, _rawAbilities, (l) => _rawAbilities = l),
