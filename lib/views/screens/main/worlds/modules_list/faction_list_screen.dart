...
final factionListStreamProvider = StreamProvider.family
    .autoDispose<List<FactionEntity>, String>((ref, worldLocalId) {
      return ref
          .watch(factionRepositoryProvider)
          .watchFactionsInWorld(worldLocalId);
    });

final factionSyncProvider = FutureProvider.autoDispose
    .family<void, ({String worldLocalId, String? worldServerId})>((
      ...
      if (ids.worldServerId != null) {
        ...
        await syncService.syncDirtyFactions();
        await syncService.fetchAndMergeFactions(
          ids.worldLocalId,
          ids.worldServerId!,
        );
      }
...
class FactionListScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final asyncFactions = ref.watch(factionListStreamProvider(worldLocalId));
            ...
            return RefreshIndicator(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _buildFactionCard(context, ref, faction);
                },
              ),
            );
          }
            ...
            error: (e, st) {
              return Center(
                child: Padding(
                  ...
                  child: Text('Failed to load factions: $e'),
                ),
              );
            },
          );
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Create your first faction to populate your world!',
            ...
            ElevatedButton.icon(
              icon: const Icon(Icons.add),
              label: const Text('Create a Faction'),
              onPressed: () {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => FactionFormScreen(
  ...
  Widget _buildFactionCard(
    ...
    final color = _getTagColor(faction.tagColor);
      ...
      child: Dismissible(
        ...
        confirmDismiss: (direction) async {
          if (direction == DismissDirection.startToEnd) {
            return await _showDeleteConfirmation(context, faction.name);
          ...
          if (direction == DismissDirection.startToEnd) {
            try {
              ref
                  ...
                  .deleteFaction(faction.localId);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('"${faction.name}" was deleted.')),
              );
            } catch (e) {
        ...
        child: ListTile(
          ...
          subtitle: Text(
            faction.customNotes ?? faction.description ?? 'No notes or description',
            maxLines: 1,
          ...
          onTap: () {
            if (faction.serverId != null) {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => FactionDetailScreen(
                    factionServerId: faction.serverId!,
                  ),
                ),
              );
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('This faction has not been synced yet.'),
