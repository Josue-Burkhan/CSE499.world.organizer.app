...
final eventListStreamProvider = StreamProvider.family
    .autoDispose<List<EventEntity>, String>((ref, worldLocalId) {
      return ref
          .watch(eventRepositoryProvider)
          .watchEventsInWorld(worldLocalId);
    });

final eventSyncProvider = FutureProvider.autoDispose
    .family<void, ({String worldLocalId, String? worldServerId})>((
      ...
      if (ids.worldServerId != null) {
        ...
        await syncService.syncDirtyEvents();
        await syncService.fetchAndMergeEvents(
          ids.worldLocalId,
          ids.worldServerId!,
        );
      }
...
class EventListScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final asyncEvents = ref.watch(eventListStreamProvider(worldLocalId));

    return Scaffold(
      appBar: AppBar(
        title: Text('Events in $worldName'),
        actions: [
          IconButton(
    ...
    return Center(
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.event_outlined, size: 80, color: Colors.grey),
            ...
            const Text(
              'Create your first event to populate your world!',
  ...
  Widget _buildEventCard(
    ...
    final color = _getTagColor(event.tagColor);
      ...
      child: Dismissible(
        ...
        confirmDismiss: (direction) async {
          if (direction == DismissDirection.startToEnd) {
            return await _showDeleteConfirmation(context, event.name);
          ...
          if (direction == DismissDirection.startToEnd) {
            try {
              ref
                  ...
                  .deleteEvent(event.localId);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('"${event.name}" was deleted.')),
              );
            } catch (e) {
        ...
        child: ListTile(
          ...
          subtitle: Text(
            event.date ?? event.customNotes ?? 'No additional info',
            maxLines: 1,
          ...
          onTap: () {
            if (event.serverId != null) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('This event has not been synced yet.'),
